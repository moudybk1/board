...
export function OnboardingPath({ className }: { className?: string }) {
  return (
    <section
      ...
      className={cn(
        "relative border-t border-edge bg-surface/30 py-12 sm:py-14",
        className,
      )}
    >
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between lg:gap-12">
          ...
          <ol className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-stretch sm:gap-0">
            {ONBOARDING_STEPS.map((step, index) => (
              <li
                key={step.id}
                className="relative flex flex-1 flex-col border border-edge bg-ink/60 px-4 py-4 sm:border-l-0 sm:first:border-l"
              >
                ...
                {step.id === "deposit" ? (
                  ...
                  <span className="mt-2 text-[10px] text-parchment">
                    {step.title}
                  </span>
                )}
                <p className="mt-1 text-[8px] leading-relaxed text-muted">
                  {step.body}
                </p>
                {index < ONBOARDING_STEPS.length - 1 && (
                  <span
                    aria-hidden
                    className="absolute -right-2 top-1/2 z-10 hidden size-4 -translate-y-1/2 rotate-45 border-r border-t border-edge bg-ink sm:block"
                  />
                )}
              </li>
            ))}
