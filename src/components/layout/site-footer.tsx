...
export function SiteFooter({ className }: { className?: string }) {
  return (
    <footer
      className={cn(
        "border-t border-edge bg-ink/60 pb-[env(safe-area-inset-bottom)]",
        className,
      )}
    >
        ...
        <nav aria-label="Legal" className="flex items-center gap-4">
          {LEGAL_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="font-pixel text-[8px] uppercase tracking-wider text-muted underline-offset-4 transition-colors hover:text-parchment hover:underline"
            >
              {link.label}
            </Link>
          ))}
